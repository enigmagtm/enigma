...
export const Put = (options?: any): MethodDecorator => {
  const PutDecorator = (target: Object, property: string | symbol, descriptor: TypedPropertyDescriptor<any>): TypedPropertyDescriptor<any> | void => {
    const method = descriptor.value;
    if (method) {
      const name = `${String(property)}${RESOURCE_METHOD}`;
      options = { ...{ name }, ...options };
      HttpMethodDecorator(target, name, HttpVerb.PUT, options);
      ...
      Reflect.defineProperty(target, name, {
        ...
        value: async function (res: Response, req: Request, next: NextFunction): Promise<void> {
          ...
          await putConfig.function(this, method, args, { res, req, next, options });
        },
        writable: false
      });
    }
